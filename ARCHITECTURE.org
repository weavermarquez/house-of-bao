...
*** Rendering: React + SVG
- Pure React components with SVG rendering
- FormPreview.tsx for visual representation
- Simpler to iterate on, works across all devices
...
*** Storage
- LocalStorage for progress persistence (via Zustand)
- No backend required

** Future Considerations
*** Rendering Engine: PixiJS v8
- Canvas-based rendering for performance
- Advanced visual effects (elastic boundaries, mirror worlds)
- Touch physics and smooth animations
- PixiJS would run in React-managed canvas container
...
- Collision detection for tap/click targeting
