...
for resp in test_responses:
    print(f"  {repr(resp[:30])} -> {extract_answer(resp)}")
#+end_src
...
def format_composite_expressions(expressions: List[str]) -> str:
    """Format a list of expressions for the composite prompt."""
    lines = []
...
def extract_composite_answer(response: str, n: int) -> int:
    ...
    # Fallback: look for "count: N" or "total: N" patterns
    match = re.search(r'(?:count|total)[:\s]+(\d+)', response, re.IGNORECASE)
    if match:
        val = int(match.group(1))
        return val if 0 <= val <= n else -1

    # Last resort: find last standalone number in reasonable range
...
test_responses = [
    ("<answer>5</answer>", 8),
    ("The count is <answer>3</answer>.", 8),
    ("Count: 7", 8),
    ("After simplifying, total: 4 expressions are ().", 8),
    ("There are 6 marks remaining.", 8),
]
for resp, n in test_responses:
    print(f"  {repr(resp[:40])} -> {extract_composite_answer(resp, n)}")
#+end_src
...
** Composite Async Eval Functions

#+begin_src python
async def eval_composite_openai_async(
    case: dict,
    model: str = MODELS["openai"]
) -> Tuple[str, int, str]:
    """Evaluate composite case using OpenAI. Returns (response, answer, model)."""
    if not OPENAI_API_KEY:
        return "", -1, model

    expr_text = format_composite_expressions(case["expressions"])
    ...
    text = response.choices[0].message.content
    return text, extract_composite_answer(text, case["group_size"]), model


async def eval_composite_anthropic_async(
    case: dict,
    model: str = MODELS["anthropic"]
) -> Tuple[str, int, str]:
    """Evaluate composite case using Anthropic. Returns (response, answer, model)."""
    if not ANTHROPIC_API_KEY:
        return "", -1, model
...
async def eval_composite_google_async(
    case: dict,
    model: str = MODELS["google"]
) -> Tuple[str, int, str]:
    """Evaluate composite case using Google. Returns (response, answer, model)."""
    if not GEMINI_API_KEY:
        return "", -1, model
...
print("Composite async eval functions defined.")
#+end_src

** Composite Sample Run

#+begin_src python :async
COMPOSITE_PROVIDERS = [
    ("openai", OPENAI_API_KEY, eval_composite_openai_async),
    ("anthropic", ANTHROPIC_API_KEY, eval_composite_anthropic_async),
    ("google", GEMINI_API_KEY, eval_composite_google_async),
]


async def run_composite_sample_async(cases: List[dict], n: int = 5) -> List[dict]:
    """Run async evaluation on composite sample cases."""
    sample = cases[:n]
    results = []
    ...
    for case in sample:
        for name, key, fn in COMPOSITE_PROVIDERS:
            if key:
                tasks.append(fn(case))
                task_info.append((case, name))
    ...
    for (case, provider), resp in zip(task_info, responses):
        if isinstance(resp, Exception):
            response, answer, model = f"ERROR: {resp}", -1, "unknown"
        else:
            response, answer, model = resp
        ...
        results.append({
            ...
            "response": response,
            "extracted_answer": answer,
            "correct": answer == case["count"]
        })

    return results


# Run composite sample evaluation
print("Running composite sample evaluation...")
composite_results = asyncio.run(run_composite_sample_async(composite_cases, n=5))

# Save results
with open("composite_sample_results.json", "w") as f:
    json.dump(composite_results, f, indent=2)
print(f"Saved {len(composite_results)} results to composite_sample_results.json")

# Display summary
print("\n=== Composite Sample Results ===")
for r in composite_results:
    status = "✅" if r["correct"] else "❌"
    print(f"{status} {r['id']} ({r['provider']}): answer={r['extracted_answer']} (target: {r['target_count']})")

# Summary stats
correct = sum(1 for r in composite_results if r["correct"])
print(f"\nOverall: {correct}/{len(composite_results)} ({100*correct/len(composite_results):.1f}%)")

# Compare to random baseline
n_problems = composite_cases[0]["group_size"]
random_baseline = 100 / (n_problems + 1)
print(f"Random baseline for {n_problems} problems: {random_baseline:.1f}%")
#+end_src

** Analyze Composite Results

#+begin_src python
def analyze_composite_results(results: List[dict]) -> dict:
    """Compute accuracy metrics for composite results."""
    from collections import defaultdict
    ...
    for r in results:
        ...
        if r["correct"]:
            ...
            # Track how far off the answer was
            if r["extracted_answer"] >= 0:
    ...
    return {
        "by_model": {m: {
            ...
            "correct": d["correct"],
            "total": d["total"],
            "avg_off_by": sum(d["off_by"])/len(d["off_by"]) if d["off_by"] else 0
        } for m, d in by_model.items()},
...
# Analyze if we have results
if 'composite_results' in dir() and composite_results:
    comp_analysis = analyze_composite_results(composite_results)

    print("=== Composite Accuracy by Model ===")
    for model, stats in comp_analysis["by_model"].items():
        pct = stats["accuracy"] * 100
        off = stats["avg_off_by"]
        print(f"{model}: {pct:.1f}% ({stats['correct']}/{stats['total']}), avg off by: {off:.1f}")

    print("\n=== Composite Accuracy by Difficulty ===")
    for model, diffs in comp_analysis["by_difficulty"].items():
        print(f"\n{model}:")
        for diff, stats in sorted(diffs.items()):
            pct = stats["accuracy"] * 100
            print(f"  {diff}: {pct:.1f}% ({stats['correct']}/{stats['total']})")
