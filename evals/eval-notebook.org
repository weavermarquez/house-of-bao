...
def create_gemini_batch(cases: List[dict], model: str = "gemini-2.5-pro") -> str:
    """Create Gemini batch job. Returns operation name."""
    # Gemini batch API uses REST
    ...
    return response.json().get("name", "gemini_batch")
...
:   OpenAI batch ID: batch_69337346124c8190a26b7070e3ceeee9
: Submitting Anthropic batch...
:   Anthropic batch ID: msgbatch_01ABY52ACx76R3152rfoYNtw
: Submitting Gemini batch...
:   Gemini batch failed: 400 Client Error: Bad Request for url: https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-pro:batchGenerateContent?key=AIzaSyB4iP4f0IBFs-e5Ny-wMiBqUffICSsn1bw
: 
: Batch status saved to batch_status.json
...
def check_gemini_batch(operation_name: str) -> Tuple[str, Optional[List[dict]]]:
    """Check Gemini batch status. Returns (status, results_or_none)."""
    # Gemini batch results are returned inline for small batches
    # For larger batches, would need to poll operation status
    return "completed", None  # Placeholder - Gemini may return results inline
