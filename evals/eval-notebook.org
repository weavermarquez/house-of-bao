...
#+author: Valerie Kim
#+property: header-args:python :session lof :results output
...
#+begin_src python
def generate_form(max_depth: int = 3, max_width: int = 3, depth: int = 0) -> list:
    """
    Generate a random LoF form.
    ...
    # Decide how many forms at this level
    ...
    for _ in range(width):
        # Each child is a mark containing a recursive form
        child_form = generate_form(max_depth, max_width, depth + 1)
...
random.seed(42)
print("Sample generated forms:")
for i in range(10):
    form = generate_form(max_depth=3, max_width=2)
    s = form_to_string(form)
    print(f"  {i+1:2}. {s if s else '<void>'}")
#+end_src
...
def simplify_step(form: list) -> Tuple[list, Optional[str]]:
    ...
    """
    # I1 (Number): Multiple adjacent marks condense to one
    # Check if we have multiple identical marks at any level
    if len(form) > 1:
        # All are marks (non-empty lists containing empty or recursively)
        # For I1: ()() = () means [[], []] -> [[]]
        # But actually I1 applies to any adjacent forms: aa -> a
        # In the simplest reading: ()() = () means two marks become one
        # We'll simplify: if we have 2+ elements, reduce to 1 (condense)
        # This is a simplification - proper I1 is about identical adjacent forms
        new_form = [form[0]]  # Keep first, drop duplicates
        return new_form, "I1"

    # I2 (Order): (()) = void, i.e., [[[]]] -> []
    if len(form) == 1 and len(form[0]) == 1 and form[0][0] == []:
        # We have [[[]]] which is (()) - cancel to void
        return [], "I2"
...
def generate_test_cases(n: int = 100, seed: int = 2024) -> List[dict]:
    ...
    # Distribution: 30% depth 1-2 (easy), 40% depth 2-3 (medium), 30% depth 3-4 (hard)
    difficulties = (
        [("easy", 2, 2)] * 30 +
        [("medium", 3, 2)] * 40 +
        [("hard", 4, 3)] * 30
    ...
    for i, (diff, max_d, max_w) in enumerate(difficulties[:n]):
        form = generate_form(max_depth=max_d, max_width=max_w)
        input_str = form_to_string(form) if form else "void"

        # Skip void inputs (boring)
        if not input_str or input_str == "void":
            form = [[]]  # At minimum, a single mark
            input_str = "()"
...
for case in test_cases[:5]:
    print(f"  {case['id']}: {case['input']:20} -> {case['target']:6} ({case['difficulty']}, {case['steps']} steps)")
#+end_src
...
def eval_openai(expression: str, model: str = "gpt-4o") -> Tuple[str, str]:
...
def eval_anthropic(expression: str, model: str = "claude-sonnet-4-20250514") -> Tuple[str, str]:
...
def run_eval(test_cases: List[dict], models: List[str] = None) -> List[dict]:
    ...
    if models is None:
        models = []
        if openai_client:
            models.append(("openai", "gpt-4o", eval_openai))
        if anthropic_client:
            models.append(("anthropic", "claude-sonnet-4-20250514", eval_anthropic))
        if google_model:
            models.append(("google", "gemini-2.0-flash", eval_google))
